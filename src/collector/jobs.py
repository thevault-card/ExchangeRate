...
FX_BACKFILL_PROGRESS_EVERY = 50  # 이 건수마다 진행 로그
...
def run_index(index_code: str, *, now: datetime, lookback_days: int = 5,
    ...
    # 판정은 짧은 커넥션에서 끝내고 닫는다. 네트워크 대기 동안 공유 DB 의 커넥션과
    # 읽기 트랜잭션을 붙들고 있지 않기 위해서다. (vaultdb 직접 적재 대비)
...
def run_fx(*, now: datetime, lookback_days: int = FX_LOOKBACK_DAYS) -> int:
    ...
    # 판정 -> (커넥션 닫고) 수집 -> 적재 순서다. 네트워크 대기 동안 공유 DB 의
    # 커넥션과 읽기 트랜잭션을 붙들지 않기 위해서다. (vaultdb 직접 적재 대비)
    with db.connect() as conn:
        # 받을 게 없으면 외부 호출 없이 끝낸다. 수출입은행은 일일 호출 한도가 있어
        # 헛호출을 아끼는 것이 그대로 이득이다.
        pending = _pending_fx_dates(conn, now, lookback_days)
        latest = db.latest_date(conn, FX_TABLE, "currency_code",
                                FX_CURRENCY_CODES[0], "rate_date")
    if not pending:
        _log(job=job, status=JobStatus.UP_TO_DATE.value, latest_date=latest)
        return 0

    collected: list[sources.FxRow] = []
    stopped_reason: str | None = None
    for rate_date in pending:
        try:
            collected.extend(sources.fetch_fx(rate_date))
    ...
    with db.connect() as conn:
        ...
        for code, row_date, base_rate, source in collected:
...
def run_fx_backfill(*, now: datetime, days: int, sleep_seconds: float = FX_BACKFILL_SLEEP_SECONDS) -> int:
    ...
    with db.connect() as conn:
        ...
        for d in pending:
            ...
            # 날짜마다 커밋한다. 백필은 몇 시간씩 도는데 묶어서 커밋하면 그동안
            # 트랜잭션이 열린 채로 남아 공유 DB 의 vacuum 을 막는다.
            conn.commit()
