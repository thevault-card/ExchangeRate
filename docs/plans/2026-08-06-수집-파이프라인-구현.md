...
- [ ] **Step 2: 전용 데이터베이스에 적용**

수집 데이터는 **기존 DB가 아니라 전용 데이터베이스에 담는다**(2026-08-06 결정). 기존 서버는 그대로 두고 데이터베이스만 하나 더 쓴다.

사용자가 DBeaver에서 먼저 만들어 둔다:

```sql
CREATE DATABASE exchangerate_dev;
```

그 다음 `.env` 의 `DATABASE_URL` 이 이 데이터베이스를 가리키게 하고, `schema.sql` 을 실행한다:

```bash
uv run --env-file .env python -c "import os,psycopg,pathlib; psycopg.connect(os.environ['DATABASE_URL'], autocommit=True).execute(pathlib.Path('schema.sql').read_text(encoding='utf-8'))"
```

기대: 오류 없이 완료. 빈 데이터베이스이므로 `CREATE TABLE` 이 두 개 다 새로 만들어진다.

> 이 `schema.sql` 은 `CREATE TABLE IF NOT EXISTS` 기반이라, **테이블이 이미 있는 옛 DB에 돌리면 아무 일도 하지 않는다.** 옛 DB의 `_test` 테이블 2개는 제약이 없는 채로 남겨두고 쓰지 않는다.
...
두 번째 인자로 조회 기간(일)을 준다. 평소에는 생략해 5일이고, 초기 백필 때만 `1095`를 넘긴다.
...
지금까지는 최근 5일치만 쌓였다. 자산 비교 차트를 그리려면 과거 구간이 필요하므로 **최초 1회만** 3년치를 받는다.

Run: `uv run --env-file .env python -m collector index_spx 1095`
Run: `uv run --env-file .env python -m collector index_kospi 1095`

Expected: `"fetched"` 가 각각 700 이상 (3년 = 약 750 거래일). 시간이 좀 걸린다.
...
Expected: 두 지수 모두 `min(trade_date)` 가 약 3년 전. 건수는 코스피와 S&P500이 서로 다르다(휴일이 달라서 정상).

백필은 여기서 끝이다. 이후 cron 은 인자 없이 돌아 5일치만 갱신한다.
...
8. 3년 백필 후 두 지수 모두 `min(trade_date)` 가 약 3년 전, 각 700행 이상
